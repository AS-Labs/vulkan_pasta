...
int main(int argc, char **argv) {
	...
	char *pp_inst_layers_names[inst_layer_count];

	for(uint32_t i=0;i<inst_layer_count;i++) {
		pp_inst_layers_names[i] = pp_inst_layers[i];
	}

	inst_cre_info.ppEnabledLayerNames = (const char * const *)pp_inst_layers_names;
